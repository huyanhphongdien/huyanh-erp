...
export default function WeighingPage() {
  ...
  // 1 lệnh = 1 xe = TẤT CẢ container của lệnh → chọn lệnh là tự áp cho cả xe (không chọn từng cont).
  ...
  // Mở lại phiếu XUẤT: khôi phục lệnh điều động đã gắn (reference_id) hoặc khớp biển số
  // → hộp khoá hiện đúng lệnh, đồng bộ KL về lệnh khi hoàn tất.
  useEffect(() => {
    if (ticket?.ticket_type !== 'out' || selectedDispatchOrderId) return
    const norm = (s?: string | null) => (s || '').toUpperCase().replace(/[^A-Z0-9]/g, '')
    let orderId = (ticket as any).reference_id || ''
    if (!orderId) {
      const plate = norm(ticket.vehicle_plate)
      const m = dispatchOrders.filter(d => norm(d.tractor_plate) === plate)
      if (m.length === 1) orderId = m[0].id
    }
    if (orderId) setSelectedDispatchOrderId(orderId)
  }, [ticket, dispatchOrders, selectedDispatchOrderId])
              ...
              {ticketDirection === 'out' && !selectedTransferId && currentFacility?.can_ship_to_customer && (
                <Card size="small" title="🚚 Lệnh điều động (xuất đi cảng)" style={{ borderRadius: 12, borderColor: PRIMARY }}>
                  <Space direction="vertical" size={12} style={{ width: '100%' }}>
                    ...
                    {/* Tóm tắt container của lệnh — TỰ áp cho cả xe, không cần chọn từng cont. */}
                    {selectedDispatchOrderId && dispatchLines.length > 0 && (
                      <div style={{ padding: 10, background: '#F0FDF4', borderRadius: 8 }}>
                        <Text strong style={{ color: '#065F46', fontSize: 13 }}>
                          {dispatchLines.length} container · tổng KL kế hoạch {dispatchPlannedKg.toLocaleString()} kg
                        </Text>
                        <div style={{ marginTop: 6 }}>
                          {dispatchLines.map(l => (
                            <Tag key={l.id} color={l.actual_weight_kg != null ? 'green' : 'default'} style={{ marginBottom: 4 }}>
                              {l.container_no || '(chưa số)'}{l.grade ? ` · ${l.grade}` : ''}{l.actual_weight_kg != null ? ' · ✅ đã cân' : ''}
                            </Tag>
                          ))}
                        </div>
                        <Text type="secondary" style={{ fontSize: 11.5 }}>
                          Cân cả xe 1 lần → KL thực tự chia về từng container theo tỉ lệ kế hoạch khi hoàn tất.
                        </Text>
                      </div>
                    )}
                  </Space>
