import{s}from"./index-Ch6ucuHr.js";const A={pending:"Chờ duyệt",approved:"Đã duyệt",paid:"Đã chi",rejected:"Từ chối"},S={pending:"orange",approved:"green",paid:"blue",rejected:"red"},f=()=>{const t=new Date,r=t.getFullYear().toString().slice(-2),e=(t.getMonth()+1).toString().padStart(2,"0"),a=Math.random().toString(36).substring(2,6).toUpperCase();return`TU${r}${e}-${a}`},q={async getAdvances(t={}){const{page:r=1,pageSize:e=10,search:a,status:n,partner_id:i,deal_id:o,date_from:p,date_to:_,sort_by:g="created_at",sort_order:y="desc"}=t,u=(r-1)*e,b=u+e-1;let d=s.from("b2b_advances").select(`
        *,
