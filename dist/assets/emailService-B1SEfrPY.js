import{c as g,s}from"./index-C2nn05h9.js";const m=[["path",{d:"M3 5h.01",key:"18ugdj"}],["path",{d:"M3 12h.01",key:"nlz23k"}],["path",{d:"M3 19h.01",key:"noohij"}],["path",{d:"M8 5h13",key:"1pao27"}],["path",{d:"M8 12h13",key:"1za7za"}],["path",{d:"M8 19h13",key:"m83p4d"}]],k=g("list",m);const u=[["path",{d:"M20 18v-2a4 4 0 0 0-4-4H4",key:"5vmcpk"}],["path",{d:"m9 17-5-5 5-5",key:"nvlc11"}]],$=g("reply",u),o="https://huyanhrubber.vn",d={excellent:"Xuất sắc",good:"Tốt",average:"Trung bình",below_average:"Cần cải thiện"},f={critical:"Khẩn cấp",high:"Cao",medium:"Trung bình",low:"Thấp"},y={task_assigned:{subject:n=>`[Huy Anh ERP] Công việc mới: ${n.task_name}`,content:n=>`
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          <p><strong>Độ ưu tiên:</strong> ${f[n.priority]||"Bình thường"}</p>
    ...
    `},self_evaluation_submitted:{subject:n=>`[Huy Anh ERP] Tự đánh giá mới cần phê duyệt: ${n.task_name}`,content:n=>`
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          ${n.quality_assessment?`<p><strong>Đánh giá chất lượng:</strong> ${d[n.quality_assessment]||n.quality_assessment}</p>`:""}
        </div>
        ${n.achievements?`
    ...
    `},task_approved:{subject:n=>`[Huy Anh ERP] ✅ Công việc đã được phê duyệt: ${n.task_name}`,content:n=>`
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #ecfdf5; padding: 20px; border-radius: 8px; margin: 20px 0; border: 1px solid #10b981;">
          ...
          <p><strong>Xếp loại:</strong> ${d[n.rating]||n.rating}</p>
        </div>
        ${n.comments?`
    ...
    `}};async function x(n){console.log("🔍 [emailService] getRecipientInfo for:",n);const{data:e,error:t}=await s.from("employees").select("id, full_name, email, department_id").eq("id",n).single();if(t)return console.error("❌ [emailService] Query error:",t.message),null;if(!e)return console.error("❌ [emailService] No data returned for:",n),null;let i=null;if(e.department_id){const{data:r}=await s.from("departments").select("id, name").eq("id",e.department_id).single();i=r}return console.log("✅ [emailService] Found recipient:",e.full_name,e.email),{...e,department:i}}async function b(n){const{data:e,error:t}=await s.from("tasks").select("id, code, name, description, status, priority, progress, due_date, department_id").eq("id",n).single();if(t||!e)return console.error("❌ [emailService] Could not find task:",n),null;let i=null;if(e.department_id){const{data:r}=await s.from("departments").select("id, name").eq("id",e.department_id).single();i=r}return{...e,department:i}}async function v(n){console.log("📧 [emailService] sendNotificationEmail:",n.notification_type);try{const e=await x(n.recipient_id);if(!e||!e.email)throw new Error("Recipient not found or has no email");let t=null;n.task_id&&(t=await b(n.task_id));const i={recipient_name:e.full_name,department_name:e.department?.name,task_id:t?.id,task_code:t?.code,task_name:t?.name,task_description:t?.description,app_url:o,...n.additional_data},r=y[n.notification_type];if(!r)throw new Error(`Unknown notification type: ${n.notification_type}`);const p=r.subject(i),c=r.content(i);console.log("📧 [emailService] ====== EMAIL DEBUG ======"),console.log("📧 [emailService] To:",e.email),console.log("📧 [emailService] Subject:",p),console.log("📧 [emailService] Body length:",c?.length),console.log("📧 [emailService] Body preview:",c?.substring(0,200));const l={to:e.email,subject:p,body:c};console.log("📧 [emailService] Full payload:",JSON.stringify(l,null,2));const{data:h,error:a}=await s.functions.invoke("send-email",{body:l});if(console.log("📧 [emailService] Response data:",h),console.log("📧 [emailService] Response error:",a),a)throw console.error("❌ [emailService] Edge Function error:",a),a;return console.log("✅ [emailService] Email sent successfully to:",e.email),{success:!0,error:null}}catch(e){return console.error("❌ [emailService] Error sending notification email:",e),{success:!1,error:e}}}async function w(n,e,t,i){console.log("📧 [emailService] notifyTaskAssigned");try{const{data:r}=await s.from("employees").select("full_name").eq("id",t).single();await v({recipient_id:e,notification_type:"task_assigned",task_id:n,additional_data:{assigner_name:r?.full_name||"Không xác định",description:i?.description,priority:i?.priority,due_date:i?.due_date}})}catch(r){console.error("❌ [emailService] Error notifying task assigned:",r)}}export{k as L,$ as R,w as n,v as s};
