import{c as h,s}from"./index-G7gJJ-lL.js";const m=[["path",{d:"M3 5h.01",key:"18ugdj"}],["path",{d:"M3 12h.01",key:"nlz23k"}],["path",{d:"M3 19h.01",key:"noohij"}],["path",{d:"M8 5h13",key:"1pao27"}],["path",{d:"M8 12h13",key:"1za7za"}],["path",{d:"M8 19h13",key:"m83p4d"}]],_=h("list",m),o="https://huyanhrubber.vn",g={excellent:"Xuất sắc",good:"Tốt",average:"Trung bình",below_average:"Cần cải thiện"},u={critical:"Khẩn cấp",high:"Cao",medium:"Trung bình",low:"Thấp"},f={task_assigned:{subject:n=>`[Huy Anh ERP] Công việc mới: ${n.task_name}`,content:n=>`
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          <p><strong>Độ ưu tiên:</strong> ${u[n.priority]||"Bình thường"}</p>
    ...
    `}};async function y(n){console.log("🔍 [emailService] getRecipientInfo for:",n);const{data:e,error:t}=await s.from("employees").select("id, full_name, email, department_id").eq("id",n).single();if(t)return console.error("❌ [emailService] Query error:",t.message),null;if(!e)return console.error("❌ [emailService] No data returned for:",n),null;let i=null;if(e.department_id){const{data:r}=await s.from("departments").select("id, name").eq("id",e.department_id).single();i=r}return console.log("✅ [emailService] Found recipient:",e.full_name,e.email),{...e,department:i}}async function x(n){const{data:e,error:t}=await s.from("tasks").select("id, code, name, description, status, priority, progress, due_date, department_id").eq("id",n).single();if(t||!e)return console.error("❌ [emailService] Could not find task:",n),null;let i=null;if(e.department_id){const{data:r}=await s.from("departments").select("id, name").eq("id",e.department_id).single();i=r}return{...e,department:i}}async function b(n){console.log("📧 [emailService] sendNotificationEmail:",n.notification_type);try{const e=await y(n.recipient_id);if(!e||!e.email)throw new Error("Recipient not found or has no email");let t=null;n.task_id&&(t=await x(n.task_id));const i={recipient_name:e.full_name,department_name:e.department?.name,task_id:t?.id,task_code:t?.code,task_name:t?.name,task_description:t?.description,app_url:o,...n.additional_data},r=f[n.notification_type];if(!r)throw new Error(`Unknown notification type: ${n.notification_type}`);const p=r.subject(i),c=r.content(i);console.log("📧 [emailService] ====== EMAIL DEBUG ======"),console.log("📧 [emailService] To:",e.email),console.log("📧 [emailService] Subject:",p),console.log("📧 [emailService] Body length:",c?.length),console.log("📧 [emailService] Body preview:",c?.substring(0,200));const l={to:e.email,subject:p,body:c};console.log("📧 [emailService] Full payload:",JSON.stringify(l,null,2));const{data:d,error:a}=await s.functions.invoke("send-email",{body:l});if(console.log("📧 [emailService] Response data:",d),console.log("📧 [emailService] Response error:",a),a)throw console.error("❌ [emailService] Edge Function error:",a),a;return console.log("✅ [emailService] Email sent successfully to:",e.email),{success:!0,error:null}}catch(e){return console.error("❌ [emailService] Error sending notification email:",e),{success:!1,error:e}}}async function k(n,e,t,i){console.log("📧 [emailService] notifyTaskAssigned");try{const{data:r}=await s.from("employees").select("full_name").eq("id",t).single();await b({recipient_id:e,notification_type:"task_assigned",task_id:n,additional_data:{assigner_name:r?.full_name||"Không xác định",description:i?.description,priority:i?.priority,due_date:i?.due_date}})}catch(r){console.error("❌ [emailService] Error notifying task assigned:",r)}}export{_ as L,k as n,b as s};
