// quick-fix.js - Chạy bằng: node quick-fix.js
const fs = require('fs');
...
fixes.forEach(([filePath, find, replace]) => {
  ...
  if (fileCache[filePath].includes(find)) {
    ...
    console.log(`   "${find.substring(0, 40)}..." → fixed`);
    totalFixes++;
  }
});

// Write all changes
Object.keys(fileCache).forEach(filePath => {
  const fullPath = path.join(process.cwd(), filePath);
