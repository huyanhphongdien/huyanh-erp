...
export function LeaveRequestListPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => leaveRequestService.getAll({
      ...
      // Manager chỉ xem phòng ban mình (cast to any vì PaginationParams chưa có department_id)
      ...(isManager && user?.department_id ? { department_id: user.department_id } : {}),
    })
  })
                          ...
                          <button
                            onClick={async () => {
                              if (confirm('Bạn có chắc muốn hủy đơn này?')) {
                                try {
                                  await leaveRequestService.cancel(req.id)
                                  queryClient.invalidateQueries({ queryKey: ['leave-requests'] })
                                } catch (err: any) {
