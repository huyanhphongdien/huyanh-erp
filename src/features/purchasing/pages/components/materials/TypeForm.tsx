...
export default function TypeForm({ 
  ...
  useEffect(() => {
    if (selectedCategory && formData.code) {
      setPreviewCode(`${(selectedCategory as any).code ?? selectedCategory.name}-${formData.code}XXX`)
    } else if (selectedCategory) {
      setPreviewCode(`${(selectedCategory as any).code ?? selectedCategory.name}-???XXX`)
    } else {
      setPreviewCode('???-???XXX')
    }
                ...
                >
                  ...
                  {categories.map(cat => (
                    <option key={cat.id} value={cat.id}>
                      {(cat as any).code ? `${(cat as any).code} - ` : ''}{cat.name}
                    </option>
                  ))}
                </select>
                {isEditing && (
              ...
              <div>
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  <div 
                    ...
                    style={{ backgroundColor: (selectedCategory as any)?.color || '#6B7280' }}
                  >
