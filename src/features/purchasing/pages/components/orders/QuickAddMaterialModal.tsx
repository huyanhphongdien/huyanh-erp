...
// ============================================================================
// THỐNG NHẤT với MaterialForm.tsx:
// - Dùng materialService.create() + generateMaterialCode()
// - Dùng previewMaterialCode() để hiện preview mã
// - Cùng fields: name, category, type, unit, supplier, specs, brand, origin...
// - Code format: [NHÓM]-[LOẠI][SỐ] VD: NL-CS001
// ============================================================================
...
export function QuickAddMaterialModal({ initialName = '', onSave, onClose }: QuickAddMaterialModalProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true)
        const [catRes, typeRes, unitRes, supRes] = await Promise.all([
          materialCategoryService.getAll({ page: 1, pageSize: 100, is_active: true }),
          materialTypeService.getAll({ page: 1, pageSize: 200, is_active: true }),
          unitService.getAll({ page: 1, pageSize: 200, isActive: true }),
          supplierService.getAll({ page: 1, pageSize: 200, status: 'active' }),
        ])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(err.message || 'Lỗi tạo vật tư. Vui lòng thử lại.')
    } finally {
      setSaving(false)
    }
  }
