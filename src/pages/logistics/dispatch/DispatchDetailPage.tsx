            ...
            <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px dashed #BAE6FD' }}>
              ...
              <Space size={28} wrap>
                <span>📦 Pallet: đi <Text strong>{order.pallet_plastic_out || 0}N+{order.pallet_steel_out || 0}S</Text>
                  {' → '}rời TL <Text strong>{(order.pallet_plastic_return != null || order.pallet_steel_return != null) ? `${order.pallet_plastic_return || 0}N+${order.pallet_steel_return || 0}S${order.fetch_tl_skipped ? ' 🚛' : ''}` : (order.fetch_tl_skipped ? '🚛 container' : '⏳')}</Text>
